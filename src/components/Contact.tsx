...
const Contact = () => {
  ...
  const getIconComponent = (iconName: string) => {
    switch (iconName) {
      ...
      case 'feather': return <Send size={22} />;
      case 'bar-chart': return <BarChart size={22} />;
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  {socialLinks.map((link) => (
                    <a
                      ...
                      className="flex flex-col items-center gap-2 p-4 rounded-lg bg-secondary/50 border border-white/5 hover:bg-primary/10 hover:border-primary/40 transition-all group"
                      aria-label={link.name}
                    >
                      <div className="p-3 rounded-full bg-background text-foreground/80 group-hover:text-primary group-hover:scale-110 transition-all">
